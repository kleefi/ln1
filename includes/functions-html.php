...
function yourls_html_head( $context = 'index', $title = '' ) {
	...
	// Page title
	$_title = 'LN1 &mdash; Shorten URL Linknet Enterprise | ' . yourls_link();
...
function yourls_html_footer($can_query = true) {
	...
	<footer id="footer" role="contentinfo"><p>
		...
		echo '&copy; '.date('Y').' PT Link Net Tbk';
		?>
	</p></footer>
...
<!-- add qr code -->
<!-- editedy by charles simanjuntak -->
	<script type="text/javascript" src="../js/easy.qrcode.js"></script>
<script>
function genQR(linkqr,elem){
	...
	new QRCode(document.getElementById(elem), {
		text: linkqr,
		width: 200,
		height: 200,
		colorDark: "#FFA500",
		...
		logo:"/user/plugins/sleeky-backend/assets/img/ln_qr_code.png",
		logoWidth:80, //
		logoHeight:80,
...
function yourls_table_add_row( $keyword, $url, $title, $ip, $clicks, $timestamp ) {
	...
	$actions = array(
		'qr_code' => array(
			...
			'title'   => "Generate QR",
			'anchor'  => "QR",
			'onclick'  => "document.getElementById('qr-$id').style.display='none';genQR('$shorturl','actions-$id');",
		),
		'stats' => array(
			'href'    => $statlink,
			'id'      => "statlink-$id",
